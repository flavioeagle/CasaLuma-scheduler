    ...
    <div>
      ...
      <div style={{background:"#fff",borderRadius:14,border:"1px solid #E5E7EB",padding:18,marginBottom:14}}>
        ...
        {adminPhotos.length===0
          ? <div style={{fontSize:13,color:"#aaa"}}>Nenhuma foto.</div>
          : <PhotoUpload label="" photos={adminPhotos} readOnly/>
        }
        {role===ROLES.ADMIN && (
          <PhotoUpload label="Adicionar foto" photos={[]} onAdd={f=>onAddPhoto(job.id,"admin",f)} uploading={uploading}/>
        )}
      </div>
      ...
      <div style={{background:"#fff",borderRadius:14,border:"2px solid #16A34A33",padding:18,marginBottom:14}}>
        ...
        <PhotoUpload label="Fotos após conclusão" photos={completionPhotos}
          onAdd={f=>onAddPhoto(job.id,"completion",f)} uploading={uploading}/>
        <div style={{marginTop:14}}>
          ...
          <textarea value={notes} onChange={e=>setNotes(e.target.value)} rows={4}
            ...
            style={{width:"100%",border:"1px solid #E5E7EB",borderRadius:8,
              padding:"10px 12px",fontSize:14,resize:"vertical",fontFamily:"inherit",boxSizing:"border-box"}}/>
...
export default function App() {
  ...
  const loadJobs = async () => {
    setLoading(true);
    const {data:jobsData,error} = await supabase.from("jobs").select("*").order("date",{ascending:true});
  ...
  const generateShare = (job) => {
    const link = `${window.location.origin}?job=${job.id}&role=installer`;
    setShareLink(link);
        ...
        <div style={{position:"fixed",inset:0,background:"#00000066",
          ...
          <div style={{background:"#fff",borderRadius:16,padding:26,maxWidth:420,width:"90%",
            ...
            <div style={{background:"#F3F4F6",borderRadius:8,padding:"10px 12px",
              fontSize:11,color:"#374151",wordBreak:"break-all",border:"1px solid #E5E7EB",marginBottom:14}}>
              {shareLink}
            </div>
